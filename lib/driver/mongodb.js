...
var MongodbDriver = Base.extend({
  ...
  /**
   * Update a record(s) into a collection
   *
   * @param collectionName  - The collection to update into
   * @param query           - Query to match
   * @param toUpdate        - The record(s) to update
   * @param callback
   */
  update: function(collectionName, query, toUpdate, callback) {
    return this._run('update', collectionName, {query: query, toUpdate: toUpdate}, callback);
  },
    ...
    this.connection.connect(connectionString, function(err, db) {
      ...
      switch(command) {
        ...
        case 'insert':
          ...
          break;
        case 'update':
          db.collection(collection)[command](options.query, options.toUpdate, callbackFunction);
          break;
        case 'remove':
          // options is the records to insert in this case
